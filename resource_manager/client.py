...
class ResourceManagerClient(object):
    ...
    def __init__(self, resource_type='machines', endpoint='http://127.0.0.1:5000'):
    ...
    def create_resource(self, resource):
        resource = requests.post(self.endpoint, data=resource, headers=self.headers)
    ...
    def update_resource(self, resource):
        ...
        updated_resource = requests.patch(request_url, data=resource, headers=self.headers)
    ...
    def delete_resource(self, resource):
        ...
        headers = self.headers
        headers['If-Match'] = etag
        deleted_resource = requests.delete(request_url, data=resource, headers=self.headers)
        if deleted_resource.status_code != 200:
            raise RequestFailureException(deleted_resource)

    def print_resources(self):
        items = requests.get(self.endpoint).json()["_items"]
